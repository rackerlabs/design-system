...
window.addEventListener('WebComponentsReady', function () {
    const tagName = 'hx-drawer';
    const template = document.createElement('template');

    template.innerHTML = `
      <style>${require('./FakeHxDrawer.less')}</style>
      ${require('./FakeHxDrawer.html')}
    `;

    class FakeHxDrawer extends HTMLElement {
        static get is () {
            return tagName;
        }
    ...
    customElements.define(FakeHxDrawer.is, FakeHxDrawer);
});
