...
### When to use
Date picker is often used for applying date range filters to lists of events, logs, or even for filtering monitoring reports.  It can also be used for advanced filtering in areas such as ticketing and billing.  The picker may be used to specify dates or ranges in the process of submitting a form for use cases other than filtering as well.
 ...
 * Move between the month selector and date selector states.

When the Date picker is triggered, it should default to the current month and year
...
### Button states
Text and icons should always be centered vertically and horizontally.
...
### Date picker spacing
The date picker element appears below corresponding text input that controls it. 
...
Month picker state maintains the same spacing considerations from Date selector state, with slight modifications of color values.
...
Button text in the month picker is always horizontally and vertically centered, and all uppercase.
